...
class myGDU(nn.Module):
    ...
    def forward(self, x, z=None, h=None):
        if self.gdu_type == 'gcn_gat':
            return self.forward_gcn_gat(x, z, h)
        elif self.gdu_type == 'original':
            return self.forward_original(x, z, h)
        elif self.gdu_type == 'simplified':
            return self.forward_simplified(x, z, h)
        elif self.gdu_type == 'extreme':
            return self.forward_extreme(x, z, h)

    # --------------------------------------------------------------------
    # ---- GDU architecture 0: returns z, it reduces DifNet to GCN/GAT ----

    def forward_gcn_gat(self, x, z, h):

        return torch.mm(z, self.W_uz) + self.b_u


    # --------------------------------------------------------------------
    # ---- GDU architecture 1: one single selection gate g ----

    def forward_extreme(self, x, z, h):
        ...
        output = torch.tanh(g * o_1 + (1 - g) * o_2 + torch.mm(x, self.W_ux) + self.b_u)
    ...
    def forward_simplified(self, x, z, h):
        ...
        output = torch.tanh(g * r * o_1 + (1 - g) * r * o_2 + g * (1 - r) * o_3 + (1 - g) * (1 - r) * o_4 + torch.mm(x, self.W_ux) + self.b_u)
